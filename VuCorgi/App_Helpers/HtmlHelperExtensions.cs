...
namespace VuBongBongWeb
{
    public static class HtmlHelperExtensions
    {
        ...
        public static IHtmlString DisplayMenu(this HtmlHelper helper, string attribute)
        {
            IEnumerable<MenuItem> menuItems = new List<MenuItem>();
            string menuDisplay = string.Empty;
            if (HttpContext.Current.User != null && HttpContext.Current.User.Identity.IsAuthenticated)
            {
                try
                {
                    var user = HttpContext.Current.User as WebPrincipal;
                    menuItems = HttpContext.Current.Session[Resource.K_USER_MENU_LIST.ToString()] as IEnumerable<MenuItem>;
                    ...
                    if (menuItems == null)
                    {
                        ...
                        {
                            data = _repo.GetMenuItemListByRole(user.UserDetail.RoleId);
                        }
                        if (data != null)
                        {
                            menuItems = data as IEnumerable<MenuItem>;
                    ...
                    {
                        lowestLevel = menuItems.Min(m => m.MenuLevel);
                        menuItems = menuItems.Where(m => m.MenuLevel == lowestLevel && !m.IsBindingWithParent && m.IsActive).OrderBy(m => m.MenuOrder).ToList();
                    }
                    foreach (var menu in menuItems)
                    {
                        menuDisplay += DisplayMenuHtml(menu, helper, attribute);
                    }
                ...
                {
                    menuDisplay = ex.Message;
                }
            }
            return helper.Raw(menuDisplay);
        }

        private static string DisplayMenuHtml(MenuItem menuItem, HtmlHelper helper, string attribute)
        {
            string menuHtml = string.Empty;
            StringBuilder sb = new StringBuilder();

            if (!menuItem.IsShownOnMenuBar)
            {
                return menuHtml;
            }
            ...
            string url = "#";
            string currentAttribute = InternalRouteIf(helper, menuItem.MenuController, attribute);
            if (!menuItem.MenuAction.IsNullOrWhiteSpace() && !menuItem.IsPageBlank)
            {
                url = u.Action(menuItem.MenuAction, menuItem.MenuController);
                currentAttribute = InternalRouteIf(helper, menuItem.MenuAction, attribute);
            }
            else
            {
                var currentController =
                    (helper.ViewContext.RequestContext.RouteData.Values["controller"] ?? string.Empty).ToString().UnDash();
                var isCurrentController = menuItem.MenuController.Equals(currentController, StringComparison.InvariantCultureIgnoreCase);
                currentAttribute = isCurrentController && !menuItem.IsPageBlank ? currentAttribute : string.Empty;
            }
            sb.Append("<li class = '" + currentAttribute + "'>");
            sb.Append("<a href='" + url + "' title='" + menuItem.MenuName + "'>");
            sb.Append("<i class='" + menuItem.MenuCssClass + "'></i> ");
            sb.Append("<span class='menu-item-parent'>" + menuItem.MenuName + "</span>");
            sb.Append("</a>");

            var childMenu = menuItem.ChildMenuItems.Where(m => !m.IsBindingWithParent && m.IsActive).OrderBy(m => m.MenuOrder);
            if (childMenu.Count() > 0)
            {
                sb.Append("<ul>");
                foreach (var menu in childMenu)
                {
                    sb.Append(DisplayMenuHtml(menu, helper, attribute));
                }
                sb.Append("</ul>");
            }
            sb.Append("</li>");
        ...
        public static string InternalRouteIf(HtmlHelper helper, string value, string attribute)
        {
            var currentController =
                (helper.ViewContext.RequestContext.RouteData.Values["controller"] ?? string.Empty).ToString().UnDash();
            var currentAction =
            ...
            return hasAction || hasController ? attribute : string.Empty;
        }
